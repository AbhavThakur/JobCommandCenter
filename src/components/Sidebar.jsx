...
export default function Sidebar({ page, setPage }) {
  ...
  const sidebarContent = (
    <div style={styles.inner}>
      ...
      <div style={styles.brand}>
        <div style={styles.brandIcon}><Zap size={14} color="#2dd4bf" /></div>
            ...
            return (
              ...
              >
                <div style={{ ...styles.navIconWrap, ...(active ? styles.navIconWrapActive : {}) }}>
                  <Icon size={15} style={{ color: active ? "var(--teal)" : "var(--muted)", flexShrink: 0 }} />
                </div>
                <span>{label}</span>
        ...
        <div style={styles.userRow}>
          ...
          <div style={styles.userInfo}>
            <div style={styles.userName}>
              {user?.isOffline ? "Local Mode" : user?.email?.split("@")[0] ?? "Unknown"}
            </div>
  ...
  return (
    <>
      <button className="gos-mobile-hamburger" style={styles.hamburger} onClick={() => setMobileOpen(true)} aria-label="Open navigation">
        <Menu size={20} />
      </button>
      ...
      {mobileOpen && (
        <div style={styles.mobileOverlay} onClick={() => setMobileOpen(false)}>
          <div style={styles.mobileDrawer} onClick={(e) => e.stopPropagation()}>
            <button style={styles.mobileClose} onClick={() => setMobileOpen(false)} aria-label="Close navigation"><X size={18} /></button>
            {sidebarContent}
          </div>
        </div>
      )}
...
const styles = {
  ...
  brandIcon: {
    ...
    background: "linear-gradient(135deg, rgba(45,212,191,0.2), rgba(45,212,191,0.06))",
    border: "1px solid rgba(45,212,191,0.25)",
    display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
  },
  brandWords: { display: "flex", flexDirection: "column", gap: 1 },
  brandText: { fontFamily: "var(--sans)", fontWeight: 800, fontSize: 14, letterSpacing: "-0.02em", color: "var(--text)" },
  brandTag: { fontSize: 10, color: "var(--muted)", fontWeight: 500, letterSpacing: "0.02em" },
  nav: { flex: 1, padding: "10px 8px", display: "flex", flexDirection: "column", overflowY: "auto" },
  navSection: { display: "flex", flexDirection: "column", gap: 1 },
  navItem: {
    display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "9px 10px",
    borderRadius: 10, background: "transparent", color: "var(--text2)",
    border: "none", fontSize: 13, fontWeight: 500, fontFamily: "var(--sans)",
  ...
  navIconWrap: {
    ...
    justifyContent: "center", background: "rgba(255,255,255,0.04)", flexShrink: 0,
    transition: "all 0.15s",
  },
  navIconWrapActive: { background: "rgba(45,212,191,0.15)" },
  navActiveDot: {
    position: "absolute", right: 10, width: 5, height: 5, borderRadius: "50%",
    background: "var(--teal)", boxShadow: "0 0 6px rgba(45,212,191,0.6)",
  },
  footer: {
    padding: "12px 12px 16px",
    borderTop: "1px solid rgba(255,255,255,0.05)",
  },
  userRow: { display: "flex", alignItems: "center", gap: 9, marginBottom: 10, minWidth: 0 },
  avatar: {
  ...
  userEmail: { fontSize: 10, color: "var(--muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  logoutBtn: { flexShrink: 0, background: "transparent", border: "none", color: "var(--muted)", cursor: "pointer", padding: 4, borderRadius: 6, display: "flex", alignItems: "center", transition: "color 0.15s" },
  version: { fontSize: 10, color: "rgba(100,116,139,0.5)", fontFamily: "var(--mono)" },
  hamburger: {
    position: "fixed", top: 14, left: 14, zIndex: 90, width: 38, height: 38,
    borderRadius: 9, background: "rgba(15,18,25,0.9)", backdropFilter: "blur(8px)",
  ...
  mobileDrawer: {
    ...
    position: "relative", animation: "drawerSlideIn 0.2s ease-out",
  },
  mobileClose: {
    position: "absolute", top: 14, right: 10, background: "transparent",
    border: "none", color: "var(--muted)", cursor: "pointer", padding: 4, borderRadius: 6,
  },
};
